...
class FlashAttentionBMMScaleBiasSoftmaxBMM : public MultiHeadedAttentionTest {
 protected:
  ...
  const std::string  // NOLINT
  GetModuleFlash_Attention_BMM1_Bias_Softmax_BMM2_Broadcast_Bias_HloString_BF16() {  // NOLINT
    const std::string hlo_text = R"(
    HloModule jit__unnamed_wrapped_function_, entry_computation_layout={(bf16[2,6,2048,128]{3,2,1,0},bf16[2,6,128,2048]{3,2,1,0},bf16[2,6,2048,128]{3,2,1,0},bf16[6,2048,2048]{2,1,0})->bf16[2,6,2048,128]{3,2,1,0}}, allow_spmd_sharding_propagation_to_output={true}
    ...
    ENTRY main.52 {
      ...
      broadcast.7 = bf16[2,6,2048,2048]{3,2,1,0} broadcast(constant.6), dimensions={}
      multiply.11 = bf16[2,6,2048,2048]{3,2,1,0} multiply(dot.10, broadcast.7)
      Arg_3.4 = bf16[6,2048,2048]{2,1,0} parameter(3), sharding={replicated}
      broadcast.1 = bf16[2,6,2048,2048]{3,2,1,0} broadcast(Arg_3.4), dimensions={1,2,3}
      add.27 = bf16[2,6,2048,2048]{3,2,1,0} add(multiply.11, broadcast.1)
      constant.9 = bf16[] constant(-inf)
      reduce.32 = bf16[2,6,2048]{2,1,0} reduce(add.27, constant.9), dimensions={3}, to_apply=region_0.28
      reshape.33 = bf16[2,6,2048,1]{3,2,1,0} reshape(reduce.32)
      broadcast.34 = bf16[2,6,2048,1]{3,2,1,0} broadcast(reshape.33), dimensions={0,1,2,3}
  ...
  template <typename T>
  void TestImpl_Flash_Attention_BMM1_Bias_Softmax_BMM2_Broadcast_Bias() {
    stream_executor::CudaComputeCapability cc = GetCudaComputeCapability();
    se::dnn::VersionInfo real_cudnn_version = GetCudnnVersion();
    if (!(cc.IsAtLeast(se::CudaComputeCapability::AMPERE) && cc.minor == 0 &&
          real_cudnn_version >= se::dnn::VersionInfo(8, 9, 3))) {
    ...
    auto rhs_bmm2_literal =
        GetInput4DLiteral<T>({2, 6, 2048, 128}, {3, 2, 1, 0});
    auto bias_literal = GetInput3DLiteral<T>({6, 2048, 2048}, {2, 1, 0});
    std::string hlo_string = "";
    hlo_string =
        GetModuleFlash_Attention_BMM1_Bias_Softmax_BMM2_Broadcast_Bias_HloString_BF16(); // NOLINT
    ExecuteAndCompare(hlo_string, {&lhs_bmm1_literal, &rhs_bmm1_literal,
...
XLA_TEST_F(FlashAttentionBMMScaleBiasSoftmaxBMM,
           Flash_Attention_BMM1_Bias_Softmax_BMM2_BF16_Broadcast_Bias) {
  TestImpl_Flash_Attention_BMM1_Bias_Softmax_BMM2_Broadcast_Bias<bfloat16>();
}
}  // namespace gpu
}  // namespace xla
