...
ENTRY main.31 {
  ...
  CudnnFusedMHARewriter fusedMhaRewriterWithCrossAttention{GetCudaComputeCapability(),
                                         GetCudnnVersionWithFlashCrossAttentionSupport()};
  TF_ASSERT_OK(RunHloPass(&fusedMhaRewriterWithCrossAttention, m.get()).status());
  SCOPED_TRACE(m->ToString());
  const HloInstruction* fmha;
  EXPECT_THAT(m->entry_computation()->root_instruction(),
              GmockMatch(m::GetTupleElement(
...
TEST_F(CudnnFusedMhaRewriterTestHloTest,
       FlashAttentionF16Bmm1BiasSoftmaxBmm2PatternBroadcastBias) {
  const char* module_str = R"(
HloModule jit__unnamed_wrapped_function_, entry_computation_layout={(f16[2,6,2048,64]{3,2,1,0},f16[2,6,64,2048]{3,2,1,0},f16[2,6,2048,64]{3,2,1,0},f16[6,2048,2048]{2,1,0})->f16[2,6,2048,64]{3,2,1,0}}
...
ENTRY main.31 {
  Arg_0.1 = f16[2,6,2048,64]{3,2,1,0} parameter(0), sharding={replicated}
  Arg_1.2 = f16[2,6,64,2048]{3,2,1,0} parameter(1), sharding={replicated}
  dot = f16[2,6,2048,2048]{3,2,1,0} dot(Arg_0.1, Arg_1.2), lhs_contracting_dims={3}, rhs_contracting_dims={2}, lhs_batch_dims={0,1}, rhs_batch_dims={0,1}
  Arg_3.4 = f16[6,2048,2048]{2,1,0} parameter(3), sharding={replicated}
  broadcast.1 = f16[2,6,2048,2048]{3,2,1,0} broadcast(Arg_3.4), dimensions={1,2,3}
  add.1 = f16[2,6,2048,2048]{3,2,1,0} add(dot, broadcast.1)
  constant = f16[] constant(-inf)
  reduce.11 = f16[2,6,2048]{2,1,0} reduce(add.1, constant), dimensions={3}, to_apply=region_0.7
  broadcast.3 = f16[2,6,2048,2048]{3,2,1,0} broadcast(reduce.11), dimensions={0,1,2}
  subtract.1 = f16[2,6,2048,2048]{3,2,1,0} subtract(add.1, broadcast.3)
  exponential.1 = f16[2,6,2048,2048]{3,2,1,0} exponential(subtract.1)
  convert.1 = f32[2,6,2048,2048]{3,2,1,0} convert(exponential.1)
  constant.1 = f32[] constant(0)
  reduce.23 = f32[2,6,2048]{2,1,0} reduce(convert.1, constant.1), dimensions={3}, to_apply=region_1.19
  convert.2 = f16[2,6,2048]{2,1,0} convert(reduce.23)
  broadcast.4 = f16[2,6,2048,2048]{3,2,1,0} broadcast(convert.2), dimensions={0,1,2}
  divide = f16[2,6,2048,2048]{3,2,1,0} divide(exponential.1, broadcast.4)
  Arg_2.3 = f16[2,6,2048,64]{3,2,1,0} parameter(2), sharding={replicated}
  ROOT dot.1 = f16[2,6,2048,64]{3,2,1,0} dot(divide, Arg_2.3), lhs_contracting_dims={3}, rhs_contracting_dims={2}, lhs_batch_dims={0,1}, rhs_batch_dims={0,1}
})";
  ...
  EXPECT_THAT(m->entry_computation()->root_instruction(),
              GmockMatch(m::GetTupleElement(
                m::CustomCall(&fmha, {kCudnnfMHAScaleBiasSoftmaxCallTarget}), 0)
                .WithShape(F16, {2, 6, 2048, 64})));
  // should get bias before broadcast here
  EXPECT_THAT(fmha->operand(3),
              GmockMatch(m::Parameter(3).WithShape(F16, {6, 2048, 2048})));
  TF_ASSERT_OK_AND_ASSIGN(
      auto config, fmha->backend_config<CudnnfMHABackendConfig>());
  EXPECT_EQ(config.is_flash_attention(), true);
  EXPECT_EQ(config.is_causal_mask(), false);
