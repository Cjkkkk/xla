    ...
    for (int batch = 0; batch < batch_count; ++batch) {
      ...
      if (AsCudaDataType(type_c) == CUDA_R_16BF) {
        ...
        TF_RETURN_IF_ERROR(DoBlasInternalImpl(
            ...
            c_matrix, AsCudaDataType(type_c), ldc, AsCublasComputeType(computation_type),
            static_cast<cublasGemmAlgo_t>(algorithm)));
        ...
        return tsl::errors::Internal("Unsupported type combination for GEMM: %s and %s", blas::DataTypeString(type_a), blas::DataTypeString(type_c));
      }
    }
    TF_RETURN_IF_ERROR(PopulateProfileFromTimer(timer.get(), algorithm,
                                                output_profile_result, stream));
