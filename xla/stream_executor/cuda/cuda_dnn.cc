...
inline std::optional<cudnn_frontend::DataType_t> GetComputeDataType(
    const xla::PrimitiveType type) {
...
class ScoreModFunc {
 ...
 public:
  Tensor_t forward(
    ...
    return compile(graph, attention_score, hlo_to_cudnn, computation);
  }
  ...
  Tensor_t backward(
    ...
    return compile(graph, grad, hlo_to_cudnn, computation);
  }

  Tensor_t compile(
      Graph_t graph, Tensor_t cudnn_input,
      absl::flat_hash_map<const xla::HloInstruction*, Tensor_t> hlo_to_cudnn,
      const xla::HloComputation* computation) {
    ...
    for (const xla::HloInstruction* hlo : instructions) {
      ...
      if (xla::HloPredicateIsOp<xla::HloOpcode::kBroadcast>(hlo)) {
        hlo_to_cudnn[hlo] = operand(0);
      } else if (xla::HloPredicateIsOp<xla::HloOpcode::kConstant>(hlo)) {
        ...
        const auto attrs = cudnn_frontend::graph::Pointwise_attributes()
                               .set_mode(mode.value())
        ...
        if (hlo->operand_count() == 1) {
          ...
          if (!is_virtual(0)) {
            ...
            if (mode.value() == cudnn_frontend::PointwiseMode_t::SUB) {
              const auto neg = cudnn_frontend::graph::Pointwise_attributes()
                               .set_mode(cudnn_frontend::PointwiseMode_t::NEG)
                               .set_compute_data_type(compute_dtype.value());
              hlo_to_cudnn[hlo] = graph->pointwise(hlo_to_cudnn[hlo], neg);
            }
          } else {
...
absl::StatusOr<CudnnGraph> GetCudnnFlashAttentionOperationGraph(
  ...
  if (fwd_comp != nullptr) {
    absl::flat_hash_map<const xla::HloInstruction*, Tensor_t> hlo_to_cudnn;
    TF_RETURN_IF_ERROR(UpdateHloParameterToCudnnMap(graph, hlo_to_cudnn, fwd_comp, &next_uid));
    TF_RETURN_IF_ERROR(UpdateHloConstantToCudnnMap(graph, hlo_to_cudnn, fwd_comp));
...
absl::StatusOr<CudnnGraph> GetCudnnFlashAttentionBackwardOperationGraph(
  ...
  if (fwd_comp != nullptr && bwd_comp != nullptr) {
    ...
    TF_RETURN_IF_ERROR(UpdateHloParameterToCudnnMap(graph, fwd_hlo_to_cudnn, fwd_comp, &next_uid));
    TF_RETURN_IF_ERROR(UpdateHloConstantToCudnnMap(graph, fwd_hlo_to_cudnn, fwd_comp));
    TF_RETURN_IF_ERROR(UpdateHloConstantToCudnnMap(graph, bwd_hlo_to_cudnn, bwd_comp));
    auto score_mod = std::make_shared<ScoreModFunc>();
